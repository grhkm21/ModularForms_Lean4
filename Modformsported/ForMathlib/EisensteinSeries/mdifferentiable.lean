...
open scoped BigOperators NNReal Classical Filter Manifold
...
--
theorem holo_to_mdiff (f : ℍ → ℂ) (hf : DifferentiableOn ℂ (↑ₕf) { point : ℂ | 0 < point.im }) :
    MDifferentiable 𝓘(ℂ) 𝓘(ℂ) f := by
  -- copied from eisensteinSeries_SIF_MDifferentiable
  intro τ
  have : DifferentiableAt ℂ (↑ₕf) τ.1 := hf.differentiableAt ?_
  convert MDifferentiableAt.comp τ (DifferentiableAt.mdifferentiableAt this) τ.mdifferentiable_coe
  exact funext fun z ↦ (comp_ofComplex f z).symm
  -- there probably is a topological proof for this lol
  simp [mem_nhds_iff]
  use Metric.ball τ.1 (τ.im / 2), ?_, ?_
  · simpa using τ.prop
  · intro x hx
    cases' τ with τ hτ
    have h₁ : ‖x - τ‖ < τ.im / 2 := by simpa using hx
    have h₂ : ‖x.im - τ.im‖ ≤ ‖x - τ‖ := by convert RCLike.norm_im_le_norm _; simp
    have h₃ := lt_of_le_of_lt h₂ h₁
    have h₄ : x.im > τ.im / 2 := by
...
#check EisensteinSeries.eisensteinSeries_SIF_MDifferentiable
